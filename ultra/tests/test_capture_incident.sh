...
case "${command}" in
  list)
    [[ "${pid}" == "0xd028" && -f "${TMP_STATE}/receiver" ]] &&
      echo '{"usage_page":"ff60","pid":"d028"}' || true
    [[ "${pid}" == "0x0c60" && -f "${TMP_STATE}/wired" ]] &&
      echo '{"usage_page":"ff60","pid":"0c60"}' || true
    ;;
  dump)
    printf '%s\n' \
